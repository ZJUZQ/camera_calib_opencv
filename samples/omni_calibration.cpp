...
double computeReprojectionErrors( const vector< cv::Mat >& objectPoints,
                                  const vector< cv::Mat >& imagePoints,
...
static void help()
{
    printf("\n This is a sample for omnidirectional camera calibration.\n"
            ...
            "    [-r <rectifiedFolder>] # the folder to store rectified images\n"
    ...
    vector<Mat>& imagePoints, Size boardSize, Size& imageSize)
{
    ...
    for(int i = 0; i < n_img; ++i)
    {
        ...
        bool found = findChessboardCorners( img_gray, boardSize, points);
...
int main(int argc, char** argv)
{
    ...
    std::string rectifiedFolder = "rectified_images";
    ...
    totalAvgErr = computeReprojectionErrors(objectPoints, imagePoints, rvecs, tvecs, K, _xi, D, perViewErrors);
    std::cout << "Average reprojection errors = " << totalAvgErr << std::endl;

    for(int i = 0; i < image_list.size(); i++)
    {
        cv::Mat origin = cv::imread(image_list[i]);
        cv::Mat rectified;

        cv::Size new_size = cv::Size(origin.cols, origin.rows);
        
        cv::Mat Knew = (cv::Mat_<float>(3, 3) << new_size.width*5/12, 0, new_size.width/2,
               0, new_size.height*5/12, new_size.height/2,
               0, 0, 1);
        
        
        cv::omnidir::undistortImage(origin, rectified, K, D, xi, cv::omnidir::RECTIFY_PERSPECTIVE, Knew, new_size);
        std::size_t found = image_list[i].find_last_of("/\\");

        cv::imwrite(rectifiedFolder + image_list[i].substr(found+1), rectified);
    }
    std::cout << "Rectified images stored in " << rectifiedFolder << " !" << std::endl;
}
...
{
    cv::Mat imagePoints2;
    if (imagePoints2.type() != CV_64FC2)
        imagePoints2.convertTo(imagePoints2, CV_64FC2);
    ...
    {
        cv::omnidir::projectPoints(objectPoints[i], imagePoints2, cv::Mat(rvecs[i]), cv::Mat(tvecs[i]), cameraMatrix, xi, distCoeffs);
        err = cv::norm(imagePoints[i], imagePoints2, CV_L2);
        int n = (int)objectPoints[i].total(); // points number
        std::cout << "debug: n = " << n << std::endl;
        std::cout << "debug: err = " << err << std::endl;
